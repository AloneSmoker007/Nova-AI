...
import { createPayment, listPayments, getPayment, updatePaymentStatus, applyPaymentWebhook, verifyPaymentWebhook } from "./services/payment.service.js";
...
function bad(error) {
  return error.message.startsWith("Invalid") || error.message.includes("too long") || error.message.includes("HTTPS") || error.message.includes("Unsupported");
}

export function registerTask16Routes(app) {
  app.get("/api/payments", requireAuth, async (req, res, next) => {
    try {
      return res.status(200).json({ status: "ok", data: await listPayments(req.user.tenantId, { status: req.query.status, limit: req.query.limit }) });
    } catch (error) {
  ...
  app.post("/api/payments/webhook/:tenantId", async (req, res, next) => {
    try {
      ...
      if (!PAYMENT_WEBHOOK_SECRET || !Buffer.isBuffer(rawBody) || rawBody.length > 64 * 1024 || !verifyPaymentWebhook(rawBody, signature, PAYMENT_WEBHOOK_SECRET)) {
  ...
  app.post("/api/ocr/extract", requireAuth, express.raw({ type: Array.from(ALLOWED), limit: MAX_IMAGE_BYTES + 256 * 1024 }), async (req, res, next) => {
    try {
      ...
      const data = await extractTextFromDocument({ tenantId: req.user.tenantId, buffer: req.body, mimeType, filename, createdBy: req.user.id });
      return res.status(201).json({ status: "ok", data });
    } catch (error) {
      if (bad(error) || error.message.includes("document size")) return res.status(400).json({ status: "error", error: error.message });
      return next(error);
    }
  });
}
