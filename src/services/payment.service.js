...
function eventId(value) {
  ...
  if (!EVENT_ID.test(v)) throw new Error("Invalid payment event ID");
  return v;
}
...
export async function listPayments(tenantId, { status, limit = 100 } = {}) {
  ...
  const result = await dbPool.query(
    `SELECT * FROM payments WHERE ${where} ORDER BY created_at DESC LIMIT $${params.length}`,
    params,
  );
  return result.rows;
}
...
export async function getPayment(tenantId, id) {
  ...
  const result = await dbPool.query("SELECT * FROM payments WHERE tenant_id=$1 AND id=$2", [t, paymentId]);
  ...
  try {
    ...
    const event = await client.query(
      `INSERT INTO payment_webhook_events
         (tenant_id, provider, event_id, provider_payment_id, status)
       VALUES ($1,$2,$3,$4,$5)
