...
export async function getDashboardSummary(tenantId) {
  const [messages, conversations, ai, revenue, appointments] = await Promise.all([
    ...
    dbPool.query("SELECT COALESCE(SUM(units),0)::int AS count FROM usage_events WHERE tenant_id = $1 AND event_type IN ('ai_message','copilot_draft','ocr_document','assistant_query')", [tenantId]),
